...
os.chdir(r"C:\Users\VRYA0001\Downloads\preprocess\crop\child")
...
for idx, video in enumerate(video_list):
    ...
    while True:
        ...
        roi = image_np[0:int(height), 80:240] # Crop the pixels here!!!! for child
        if(not writerInit):
            h,w,_ = roi.shape
...
os.chdir(r"C:\Users\VRYA0001\Downloads\preprocess\crop\adult")
...
for idx, video in enumerate(video_list):
    ...
    out1 = cv2.VideoWriter(name_out, fourcc, 2.997, (int(width), int(height)))
    writerInit = False
    while True:
        ret, image_np = cap.read()
        if not ret:
